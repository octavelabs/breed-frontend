...
function SessionDetailModal({
  ...
  const statusStyle = STATUS_CLASSES[session.status] ?? {
    bg: "bg-gray-100",
    text: "text-gray-600",
  };
  const canCancel =
      ...
      >
        ...
        <div className="bg-[#870BD6] px-6 py-5 relative">
          ...
          <div className="relative flex items-start justify-between">
            ...
            <button
              onClick={onClose}
              className="p-1 rounded-full hover:bg-white/20 text-white/80 mt-0.5"
            >
              <X size={18} />
            </button>
          </div>
          <span
            className={`mt-2 inline-block text-xs font-semibold px-2.5 py-0.5 rounded-full ${statusStyle.bg} ${statusStyle.text}`}
          >
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold text-[#180426]">
                  {session.disciple.firstName} {session.disciple.lastName}
                </p>
                {session.disciple.username && (
                  <p className="text-xs text-[#60666B]">
                    @{session.disciple.username}
                  </p>
                )}
            ...
            {session.description && (
              <div className="bg-[#F8FAFC] rounded-xl p-3">
                <p className="text-xs text-[#60666B] mb-1 font-medium">
                  Description
                </p>
            ...
            {confirmCancel && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-3">
                ...
                <p className="text-xs text-red-600 mb-3">
                  Both you and {session.disciple.firstName} will be notified by
                  email.
                </p>
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={cancelling}
                    className="flex-1 h-9 text-xs border border-gray-300 rounded-full text-[#60666B] hover:bg-gray-50"
                  >
                    Keep it
                  </button>
                  <Button
...
function CalendarModal({
  ...
  const cells: (Date | null)[] = [
    ...
    ...Array.from(
      { length: daysInMonth },
      (_, i) => new Date(year, mon, i + 1),
    ),
  ];

  const hasSession = (d: Date) =>
    sessions.some((s) => isSameDay(new Date(s.scheduledAt), d));
  const daySessions = selected
    ? sessions.filter((s) => isSameDay(new Date(s.scheduledAt), selected))
    : [];
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E3E8EF]">
          <h2 className="font-semibold text-[#180426] text-base">
            Full Calendar
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="p-1.5 border border-gray-200 rounded-full hover:bg-gray-50"
            >
          ...
          <div className="grid grid-cols-7 mb-2">
            {DAY_LABELS.map((d) => (
              <div
                key={d}
                className="text-center text-xs font-medium text-[#60666B] py-1"
              >
                {d}
              </div>
            ))}
            ...
            {cells.map((date, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative w-full aspect-square rounded-lg flex flex-col items-center justify-center text-sm font-medium transition-all ${
                    isSelected
                      ...
                      : isToday
                        ? "bg-[#FBF6FF] border border-[#870BD6] text-[#870BD6]"
                ...
                >
                  ...
                  {hasDot && (
                    <span
                      className={`absolute bottom-1 w-1 h-1 rounded-full ${isSelected ? "bg-white" : "bg-[#870BD6]"}`}
                    />
                  )}
                </button>
              );
          ...
          {selected && (
            <div className="mt-5 border-t border-[#E3E8EF] pt-4">
              ...
              {daySessions.length === 0 ? (
                <p className="text-sm text-[#60666B] text-center py-4">
                  No sessions this day.
                </p>
              ) : (
                <div className="space-y-2">
                  {daySessions.map((s) => {
                    const st = STATUS_CLASSES[s.status] ?? {
                      bg: "bg-gray-100",
                      text: "text-gray-600",
                    };
                    return (
                      ...
                      >
                        ...
                        <ExternalLink
                          size={12}
                          className="text-gray-300 group-hover:text-[#870BD6] shrink-0"
                        />
                      </button>
                    );
                  })}
...
function SessionCard({
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-3 p-3 bg-[#F8FAFC] border border-[#E3E8EF] cursor-pointer rounded-xl hover:border-[#870BD6] hover:bg-[#FBF6FF] transition-all group text-left w-full"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-[#1D1B20] truncate">
          {s.title}
        </p>
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-[10px] text-[#60666B] truncate">
            · {s.disciple.firstName} {s.disciple.lastName}
          </span>
        </div>
      </div>
      <span
        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${st.bg} ${st.text}`}
      >
        {s.status.toLowerCase().replace("_", " ")}
      </span>
    </button>
...
function ScheduleSessionModal({
  ...
  useEffect(() => {
    mentorshipService
      .getDisciples({ status: "ACTIVE", limit: 50 })
      .then((res: any) => {
        const data = res?.data ?? res;
        const list: Mentee[] = (Array.isArray(data) ? data : []).map(
          (m: any) => ({
            mentorshipId: m.id,
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#60666B] mb-1">
              Mentee <span className="text-red-500">*</span>
            </label>
            {loadingMentees ? (
              <div className="h-10 bg-gray-100 rounded-lg animate-pulse" />
            ) : mentees.length === 0 ? (
              <p className="text-sm text-[#60666B] bg-[#F8FAFC] rounded-lg p-3">
              ...
              >
                ...
                {mentees.map((m) => (
                  <option key={m.mentorshipId} value={m.mentorshipId}>
          ...
          <div>
            <label className="block text-xs font-medium text-[#60666B] mb-1">
              Meeting Link <span className="text-[#A0A8B0]">(optional)</span>
            </label>
            <div className="relative">
              <Link
                size={14}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-[#A0A8B0]"
              />
              <input
                type="url"
                value={meetingLink}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button
              buttonType="custom"
              customClass="flex-1 !h-10 !text-sm bg-linear-to-b from-[#A967F1] to-[#5B26B1] text-white rounded-full"
              loading={submitting}
              onClick={() => {}}
            >
              Schedule
...
export const SessionCalendar = ({
  ...
  const daySessions = allSessions.filter((s) =>
    isSameDay(new Date(s.scheduledAt), selectedDate),
  );
  const hasSessionOnDay = (d: Date) =>
    allSessions.some((s) => isSameDay(new Date(s.scheduledAt), d));
  ...
  const goToSession = (s: Session) =>
    router.push(`/dashboard/mentorship/sessions/${s.id}`);
      ...
      <div className="bg-white border border-[#D2D9DF] rounded-[16px] w-full px-5 py-6">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-[#1D1B20] font-medium">
              {selectedLabel}
            </span>
            <hr className="flex-1 border-[#D2D9DF]" />
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center py-6 gap-1">
              <CalendarDays
                ...
                strokeWidth={1.5}
              />
              <p className="mt-2 text-sm font-medium text-[#180426]">
                No sessions yet
              </p>
              <p className="text-xs text-[#60666B]">
                Sessions you schedule with mentees will appear here.
              </p>
            </div>
          ) : (
            <div>
