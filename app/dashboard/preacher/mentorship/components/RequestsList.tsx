...
const RequestsList: React.FC = () => {
    ...
    const handleButtonClick = (request: Request, type: string) => {
        ...
        type === 'accept' ? setModalOpen({ ...isModalOpen, accept: true }) : setModalOpen({ ...isModalOpen, reject: true });
    };
    ...
    return (
        <div className="bg-white">
            {isModalOpen.accept && <AcceptModal
                isOpen={isModalOpen.accept}
                onClose={onClose}
                selectedRequest={selectedRequest}
            />}
            {isModalOpen.reject && <RejectModal
            ...
            <div className="bg-white mx-4 lg:mx-10 border border-[#E3E8EF] rounded-[16px] mb-5">
                ...
                <div className="flex items-center justify-center gap-4 py-4 border-t border-gray-200">
                    ...
                    {[1, 2, 3, '...', 8, 9, 10].map((page, index) => (
                        <button
                            key={index}
                            className={`flex justify-center items-center w-10 h-10 text-sm font-medium rounded-[8px] text-[#4E5255] ${page === 1
                                ? 'bg-[#E2E3E5]'
                                : 'bg-white'
                                }`}
                        >
