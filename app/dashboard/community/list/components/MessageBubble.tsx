...
export const MessageBubble: React.FC<BubbleMessageProps> = ({ message, isOwnMessage }) => (
  <div className={`flex gap-3 mb-8 w-[85%] lg:w-[60%] min-w-0 ${!isOwnMessage ? "mr-auto" : "ml-auto"}`}>
    ...
    <div className={`flex-1 min-w-0 ${!isOwnMessage ? "bg-[#F6F8FA]" : "bg-[#EAE1F2]"} rounded-[15px] py-2 px-[13px]`}>
      <div className="text-[10px] font-semibold mb-2">{message.author}</div>
      <div className="text-xs leading-relaxed mb-2 whitespace-pre-wrap break-words">
        {renderText(message.text)}
      </div>
      <div className="text-xs font-normal text-end">{message.time}</div>
    </div>
    {isOwnMessage && (
